...
class GridEnvSim:
    def __init__(self, w, h, rand_init, num_goals, rand_goals, task_only, rep_type):
        ...
        wind_locs = []
        for cnt_y in range(1, 10):
            for cnt_x in range(3, 5):
                wind_locs.append([cnt_y, cnt_x])
        for cnt_y in range(1, 10):
            wind_locs.append([cnt_y, 7])
        self.wind_locs = np.asarray(wind_locs)
        strong_wind_locs = []
        for cnt_y in range(1, 10):
            for cnt_x in range(5, 7):
                strong_wind_locs.append([cnt_y, cnt_x])
        self.strong_wind_locs = np.asarray(strong_wind_locs)
        if rand_init:
            ...
            self.starting_loc = np.asarray([9, 0])
        #print("starting_loc:")
    ...
    def do_action(self, action):
        if 0 == self.stage:
            ...
            if action < 4:
                ...
                if (new_pos == self.wind_locs).all(1).any():
                    new_pos[0] = max(new_pos[0]-1, 0)
                elif (new_pos == self.strong_wind_locs).all(1).any():
                    new_pos[0] = max(new_pos[0]-2, 0)
            else:
