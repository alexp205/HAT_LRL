...
# TEST
episodes = []
#overall_episodes = {"LunarLander-v2": [()]*len(file_idxs), "gridworld": [()]*len(file_idxs), "CartPole-v1": [()]*len(file_idxs)}
overall_rs = {"LunarLander-v2": [()]*len(file_idxs), "gridworld": [()]*len(file_idxs), "CartPole-v1": [()]*len(file_idxs)}
#naive_overall_episodes = {"LunarLander-v2": [()]*len(file_idxs), "gridworld": [()]*len(file_idxs), "CartPole-v1": [()]*len(file_idxs)}
naive_overall_rs = {"LunarLander-v2": [()]*len(file_idxs), "gridworld": [()]*len(file_idxs), "CartPole-v1": [()]*len(file_idxs)}
#indiv_overall_episodes = {"LunarLander-v2": [()]*len(file_idxs), "gridworld": [()]*len(file_idxs), "CartPole-v1": [()]*len(file_idxs)}
indiv_overall_rs = {"LunarLander-v2": [()]*len(file_idxs), "gridworld": [()]*len(file_idxs), "CartPole-v1": [()]*len(file_idxs)}
...
for i in file_idxs:
    ...
    for idx, j in enumerate(exper_order):
        ...
        #overall_episodes[j][i-1] = overall_episodes[j][i-1] + tuple(temp_eps)
        overall_rs[j][i-1] = overall_rs[j][i-1] + tuple(temp_rs)
        #naive_overall_episodes[j][i-1] = overall_episodes[j][i-1] + tuple(temp_eps)
        naive_overall_rs[j][i-1] = naive_overall_rs[j][i-1] + tuple(naive_temp_rs)
        #indiv_overall_episodes[j][i-1] = overall_episodes[j][i-1] + tuple(temp_eps)
        indiv_overall_rs[j][i-1] = indiv_overall_rs[j][i-1] + tuple(indiv_temp_rs)
...
overall_rs_means = []
overall_rs_stdevs = []
naive_overall_rs_means = [] 
naive_overall_rs_stdevs = [] 
indiv_overall_rs_means = []
indiv_overall_rs_stdevs = []

for idx, exper_name in enumerate(exper_order):
    r_data = overall_rs[exper_name]
    r_data = np.asarray(r_data)
    overall_rs_means.append(np.mean(r_data, axis=0))
    overall_rs_stdevs.append(np.std(r_data, axis=0))
    r_data = naive_overall_rs[exper_name]
    r_data = np.asarray(r_data)
    naive_overall_rs_means.append(np.mean(r_data, axis=0))
    naive_overall_rs_stdevs.append(np.std(r_data, axis=0))
    r_data = indiv_overall_rs[exper_name]
    r_data = np.asarray(r_data)
    indiv_overall_rs_means.append(np.mean(r_data, axis=0))
    indiv_overall_rs_stdevs.append(np.std(r_data, axis=0))

overall_rs_means = np.concatenate(overall_rs_means)
overall_rs_stdevs = np.concatenate(overall_rs_stdevs)
naive_overall_rs_means = np.concatenate(naive_overall_rs_means)
naive_overall_rs_stdevs = np.concatenate(naive_overall_rs_stdevs)
indiv_overall_rs_means = np.concatenate(indiv_overall_rs_means)
indiv_overall_rs_stdevs = np.concatenate(indiv_overall_rs_stdevs)

fig, axs = plt.subplots(1, 1, figsize=(8,8))
exper_line, = axs.plot(episodes, overall_rs_means, linestyle='solid', color='black', label="HAT")
axs.fill_between(episodes, overall_rs_means-overall_rs_stdevs/2., overall_rs_means+overall_rs_stdevs/2., alpha=0.3, edgecolor='black', facecolor='black')
naive_line, = axs.plot(episodes, naive_overall_rs_means, linestyle='dashed', color='black', label="Naive")
axs.fill_between(episodes, naive_overall_rs_means-naive_overall_rs_stdevs/2., naive_overall_rs_means+naive_overall_rs_stdevs/2., alpha=0.3, edgecolor='black', facecolor='black', linestyle='dashed')
indiv_line, = axs.plot(episodes, indiv_overall_rs_means, linestyle='dashdot', color='black', label="Base")
axs.fill_between(episodes, indiv_overall_rs_means-indiv_overall_rs_stdevs/2., indiv_overall_rs_means+indiv_overall_rs_stdevs/2., alpha=0.3, edgecolor='black', facecolor='black', linestyle='dashdot')
